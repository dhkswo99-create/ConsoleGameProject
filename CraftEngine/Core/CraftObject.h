...
namespace Craft
{
	//커스텀 타입 시스템을 제공하는 최상위클래스
	//단일 상속만 지원
	class CRAFT_API CraftObject
	{
	public:
		//가상 소멸자
		virtual ~CraftObject() = default; //상속관계에서 부모클래스들의 소멸을 담당

		// 현재 객체 타입의 ID를 반환하는 함수
		// 순수 가상 함수로 만들기 -> 상속하는 계층에 구현을 강제
		virtual size_t GetType() const = 0; //순수 가상함수 모든 자식 클래스가 구현해야함. 인터페이스//
		...
		//타입 질문 함수
		//이 함수를 사용하기 위해서는 T타입이 static TypeId 함수를 가져야만 함.
		//-> 기본 자료형은 판단 불가능.
		template<typename T>
		bool IsTypeOf() const
		{
